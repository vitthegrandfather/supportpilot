export interface SeedDocument {
  ...
  publicId: string;
  title: string;
  category: "billing" | "security" | "privacy" | "product" | "policy" | "runbook";
  version: string;
  sourceType: "markdown" | "pdf" | "txt" | "html";
  filename: string;
  ownerName: string;
  usageCount: number;
  suspicious?: boolean;
...
`,
  },
  {
    ...
    usageCount: 0,
    status: "failed",
    failedReason: "Binary PDF extractor rejected an encrypted file (owner password set). Re-upload an unencrypted PDF or paste the text.",
    body: "",
  },
];
